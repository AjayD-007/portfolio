...
export function Sidebar({
  ...
  return (
    <div className="absolute top-24 md:top-32 right-4 md:right-8 w-64 max-h-[calc(100vh-8rem)] flex flex-col bg-white/10 dark:bg-black/60 backdrop-blur-2xl border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl text-black dark:text-white pointer-events-auto overflow-hidden">
      <div className="px-5 py-4 border-b border-black/5 dark:border-white/5 flex items-center justify-between bg-white/40 dark:bg-white/5 shrink-0">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/50 dark:text-white/50 mb-1">
            Fractal Explorer
          </p>
          <p className="font-mono text-sm font-semibold">{zoomStr}</p>
        </div>
        <div
          className={`text-[9px] px-2 py-1 rounded-full flex items-center gap-1.5 font-semibold tracking-wide ${
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        {process.env.NODE_ENV === "development" && devData && !hideDebug && (
          <div className="px-5 py-3 border-b border-black/5 dark:border-white/5 bg-red-500/5 relative">
            <button onClick={() => setHideDebug(true)} className="absolute top-2 right-2 text-red-500/50 hover:text-red-500 text-[10px] font-bold px-1 rounded hover:bg-red-500/10 transition-colors">HIDE</button>
            <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-red-500 mb-2">
              Dev Debug
            </p>
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] font-mono text-black/60 dark:text-white/60">
        ...
        {process.env.NODE_ENV === "development" && devData && hideDebug && (
          <div className="px-5 py-2 border-b border-black/5 dark:border-white/5 bg-red-500/5 flex justify-between items-center cursor-pointer hover:bg-red-500/10" onClick={() => setHideDebug(false)}>
            <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-red-500/50">Dev Debug (Hidden)</p>
        ...
        <div className="flex flex-col gap-6 p-5">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/40 dark:text-white/40 mb-3">Palette</p>
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="text-[10px] text-black/60 dark:text-white/60 mb-1.5 block font-medium">Inside</label>
                <input
                  type="color"
                  value={colorA}
              ...
              <div className="flex-1">
                <label className="text-[10px] text-black/60 dark:text-white/60 mb-1.5 block font-medium">Outside</label>
                <input
                  type="color"
                  value={colorB}
          ...
          <div>
            <div className="flex justify-between items-end mb-2">
              <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/40 dark:text-white/40">
                Density
              </label>
              <span className="text-[10px] font-mono text-black/60 dark:text-white/60">{density.toFixed(1)}x</span>
            </div>
            <input
              type="range"
              min={0.1}
          ...
          <div>
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-[10px] font-mono text-black/60 dark:text-white/60">{maxIter} iter</span>
            </div>
            <input
              type="range"
              min={64}
      ...
      <div className="p-3 border-t border-black/5 dark:border-white/5 flex flex-col gap-2 bg-white/40 dark:bg-white/5 shrink-0 ">
        ...
        <div className="flex gap-2">
          <button
            onClick={onReset}
            className="flex-1 py-2 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-black/70 dark:text-white/70 text-[10px] font-bold uppercase tracking-wider transition-colors cursor-pointer"
          >
            Reset
          </button>
          <button
            onClick={onDownload}
            className="flex-[2] py-2 rounded-lg bg-black dark:bg-white hover:bg-black/80 dark:hover:bg-white/80 text-white dark:text-black shadow-lg text-[10px] font-bold uppercase tracking-wider transition-all hover:scale-[1.02] cursor-pointer"
          >
            Export PNG
          </button>
        </div>
